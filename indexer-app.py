...
def event_handler():
    ...
    if bucket_name and file_name:
        ...
        if file_name_only:
            num_pages=0 
            psql.update_file_status(id=f"{folder_name}_{file_name_only}", status="processing", page_nums=num_pages, updatedAt=datetime.now())
            oFileExtMap = {
            ...
            loop = get_event_loop()
            results = loop.run_until_complete(process_pages_async(pages, headers, filename, userid=folder_name))
            #Get embeddings

            # Upload content to pinecone index in batches with metadata
            ...
            sqlres =  psql.update_file_status(id=f"{folder_name}_{file_name_only}", status="processed", page_nums=num_pages, updatedAt=datetime.now())
...
async def process_pages_async(pages, headers, filename, userid):
    url = SERVER_URL
    ...
    await create_and_upload_embeddings_in_batches(results, filename, userid)
    return results
...
async def create_and_upload_embeddings_in_batches(results, filename, userid,  batch_size=10):
    batch = []
    texts = []
    page_nums = []
    ...
    for text_content, page_num in results:
        ...
        if len(texts) >= batch_size:
            ...
            for text, embedding, page_num in zip(texts, embeddings, page_nums):
                formatted_filename = filename.replace(' ', '-').replace(':', '-').replace('/', '-')
            ...
            upload_to_pinecone(batch, userid)
            batch.clear()
            texts.clear()
            page_nums.clear()
